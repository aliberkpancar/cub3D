...
static void	dda_algorithm(t_vars *vars)
{
	...
	hit = 0;
	printf("vars->r_map[vars->ray.map_x][vars->ray.map_y]: %c\n", vars->r_map[0][0]);
	while (hit == 0)
	{
		...
		if (vars->ray.map_x >= 0 && vars->ray.map_x < vars->height &&
			vars->ray.map_y >= 0 && vars->ray.map_y < vars->width)
			if (vars->r_map[vars->ray.map_x][vars->ray.map_y] == '1')
				hit = 1;
	}
}

#include "../mlx/mlx.h"
#include "../includes/cub3d.h"
#include <stdlib.h>

static void init_player_pos(t_vars *vars)
{
	int x;
	int y;

	y = 0;
	while (y < vars->height)
	{
		x = 0;
		while (x < vars->width)
		{
			if (vars->r_map[y][x] == 'N' || vars->r_map[y][x] == 'S' || vars->r_map[y][x] == 'W' || vars->r_map[y][x] == 'E')
			{
				vars->pos_x = x + 0.5;
				vars->pos_y = y + 0.5;
				if (vars->r_map[y][x] == 'N')
				{
					vars->dir_x = 0;
					vars->dir_y = -1;
					vars->plane_x = 0.66;
					vars->plane_y = 0;
				}
				else if (vars->r_map[y][x] == 'S')
				{
					vars->dir_x = 0;
					vars->dir_y = 1;
					vars->plane_x = -0.66;
					vars->plane_y = 0;
				}
				else if (vars->r_map[y][x] == 'W')
				{
					vars->dir_x = -1;
					vars->dir_y = 0;
					vars->plane_x = 0;
					vars->plane_y = -0.66;
				}
				else if (vars->r_map[y][x] == 'E')
				{
					vars->dir_x = 1;
					vars->dir_y = 0;
					vars->plane_x = 0;
					vars->plane_y = 0.66;
				}
				vars->r_map[y][x] = '0';
				return ;
			}
			x++;
		}
		y++;
	}
	printf("Error: Player start position not found in the map.\n");
	exit(EXIT_FAILURE);
}

static void	init_var(t_vars *vars, char *map)
{
	int i;

	vars->mlx = mlx_init();
	vars->win = mlx_new_window(vars->mlx, WIN_WIDTH, WIN_HEIGHT, "cub3D");
	vars->image.image = mlx_new_image(vars->mlx, WIN_WIDTH, WIN_HEIGHT);
	vars->image.addr = mlx_get_data_addr(vars->image.image, &vars->image.bits_per_pixel, &vars->image.line_length, &vars->image.endian);
	vars->pos_x = 22.0;
	vars->pos_y = 12.0;
	vars->dir_x = -1.0;
	vars->dir_y = 0.0;
	vars->plane_x = 0.0;
	vars->plane_y = 0.66;
	vars->temp_width = 0;
	vars->temp_height = 0;
	get_dimensions(vars, map);
	vars->temp_map = (char **)malloc(vars->temp_height * sizeof(char *)); 
	if (!vars->temp_map)
	{
		printf("Error\n");
		exit(EXIT_FAILURE);
	}
	i = 0;
	while (i < vars->temp_height)
	{
		vars->temp_map[i] = (char *)malloc(vars->temp_width + 1 * sizeof(char)); 
		if (!vars->temp_map[i])
		{
			printf("Error\n");
			exit(EXIT_FAILURE);
		}
		i++;
	}
	vars->player_count = 0;
	vars->ray.side_dist_x = 0;
    vars->ray.side_dist_y = 0;
    vars->ray.delta_dist_x = 0;
    vars->ray.delta_dist_y = 0;
    vars->ray.map_x = 0;
    vars->ray.map_y = 0;
    vars->ray.step_x = 0;
    vars->ray.step_y = 0;
    vars->ray.side = 0;
}



int main(int argc, char **argv)
{
	t_vars *vars;

	if (argc != 2)
		return (printf("Error\n"), 1);
	vars = (t_vars *)malloc(sizeof(t_vars));
	if (!vars)
	{
		printf("Error\n");
		exit(EXIT_FAILURE);
	}
	init_var(vars, argv[1]);
	texture_init(vars);
	parse_map(argv[1], vars);
	create_r_map(vars);
	check_r_map(vars);
	init_player_pos(vars);
	mlx_loop_hook(vars->mlx, main_loop, vars);
	mlx_hook(vars->win, 2, 1L<<0, handle_input, vars);
	mlx_loop(vars->mlx);
	return (0);
}
